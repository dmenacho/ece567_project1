...
class BaseWorkspace(tp.Generic[C]):
    ...
    def eval(self) -> None:
        ...
        while eval_until_episode(episode):
            ...
            vx_list = []
            target_vx = None
            # if custom_reward == 'quadruped_pos' and hasattr(custom_reward, "vx"):
            #     target_vx = np.array(custom_reward.vx, dtype=np.float32)

            # if custom_reward is not None and hasattr(custom_reward, "x"):
            #     target_vx = np.array(custom_reward.x, dtype=np.float32)
            
            if custom_reward is not None and hasattr(custom_reward, "target_speed"):
                target_vx = custom_reward.target_speed
            ...
            while not time_step.last():
                ...
                if custom_reward is not None:
                    # if custom_reward == 'quadruped_pos' and hasattr(self.eval_env.physics, "torso_velocity"):
                    #     current_vx = np.array(self.eval_env.physics.torso_velocity()[:2], dtype=np.float32)
                    #     vx_list.append(current_vx)

                    # current_vx = self.eval_env.physics.named.data.xpos['torso', 'x']
                    # vx_list.append(current_vx)

                    current_vx = self.eval_env.physics.speed()
                    vx_list.append(current_vx)
    ...
    def load_checkpoint(self, fp: tp.Union[Path, str], only: tp.Optional[tp.Sequence[str]] = None, exclude: tp.Sequence[str] = ()) -> None:
        ...
        with fp.open('rb') as f:
            payload = torch.load(f, map_location=torch.device('cpu'), weights_only=False)
