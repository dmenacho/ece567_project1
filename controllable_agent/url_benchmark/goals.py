...
def _make_env(domain: str) -> dmc.EnvWrapper:
    task = {"quadruped": "stand", "walker": "walk", "jaco": "reach_top_left", "point_mass_maze": "reach_bottom_right", "cheetah":"run"}[domain]
...
def get_reward_function(name: str, seed: tp.Optional[int] = None) -> BaseReward:
    ...
    if name == "quadruped_pos":
        return QuadrupedCase4FixedReward((0.2,0.2))
    if name=="cheetah_speed":
        return CheetahSpeedReward(1.5)
...
class CheetahSpeedReward(BaseReward):
    def __init__(self, target_speed=2.0):
        super().__init__()
        self.target_speed = target_speed
        self._env = dmc.make("cheetah_run", obs_type="states", frame_stack=1, action_repeat=1, seed=12)

    def from_env(self, env):
        v = env.physics.speed()
        return float(np.exp(-abs(v - self.target_speed)))
...
class QuadrupedCase4FixedReward(BaseReward):
    def __init__(self, target_vx=(0.6, 0.0)) -> None:
        super().__init__()
        self._env = _make_env("quadruped")
        self.vx = np.array(target_vx, dtype=np.float32)

    def from_env(self, env: dmc.EnvWrapper) -> float:
        vx = env.physics.torso_velocity()[:2]
        up = max(0.0, float(env.physics.torso_upright()))
        error = np.linalg.norm(self.vx - vx) / np.sqrt(2)
        return up * _inv(error)
